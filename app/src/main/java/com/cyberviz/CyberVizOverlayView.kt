...
class CyberVizOverlayView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private var regions: List<TextAnalyzer.TextRegion> = emptyList()
    ...
    fun setTextRegions(r: List<TextAnalyzer.TextRegion>) { regions = r; invalidate() }
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        bitmap?.let { bmp ->
            val sx = width.toFloat() / bmp.width; val sy = height.toFloat() / bmp.height
            val s = minOf(sx, sy)
            val dw = bmp.width * s; val dh = bmp.height * s
            val dl = (width - dw) / 2; val dt = (height - dh) / 2
